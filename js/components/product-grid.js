...
export async function displayProducts() {
  ...
  dataArray.forEach((product) => createProductCard(product, productGrid));
}
...
function createProductCard(product, productGrid) {
  ...
  createStars(product.rating.rate, productRatingRate);
...
function createStars(rate, span) {
  const remainder = (rate * 10) % 10;
  let stars = 0;
